...
import com.ctb.vcardtest_project.LogUtils.logE
import com.ctb.vcardtest_project.LogUtils.logW
import java.io.*
...
object FileUtils {
    ...
    private val FILE_DIR = (Environment.getExternalStorageDirectory().toString() + "/"
        ...
        try {
            ...
            if (input is FileInputStream) {
                ...
                if (!parentFile.exists() && !parentFile.mkdirs()) {
                    logW(TAG, "[MMS] copyPart: mkdirs for " + parentFile.path.toString() + " failed!")
                    return null
                }
            ...
            logE(TAG, "Exception caught while save attachment: ", e)
            return null
        } finally {
            if (null != input) {
                try {
                    ...
                    logE(TAG, "Exception caught while closing input: ", e)
                }
            }
            if (null != fout) {
                try {
                    ...
                    logE(TAG, "Exception caught while closing output: ", e)
                }
            }
        }
        return saveUri
    ...
    private fun createSaveFile(context: Context, attachmentUri: Uri, attachmentName: String): File {
        ...
        if (fileName.lastIndexOf('.').also { index = it } != -1) {
            ...
            val cursor: Cursor? = context.getContentResolver().query(attachmentUri, null, null, null, null)
            cursor?.use { cursor1 ->
                if (cursor1.moveToFirst()) {
                    val type: String = cursor1.getString(cursor1.getColumnIndexOrThrow(COLUMN_CONTENT_TYPE))
