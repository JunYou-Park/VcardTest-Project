...
public class ExportProcessor extends ProcessorBase {
    ...
    public void run() {
        ...
        try {
            ...
            FeedbackHelper.sendFeedback(mService, TAG, "Failed to process vcard export", e);
            throw e;
        } finally {
            synchronized (this) {
                mDone = true;
    ...
    private void runInternal() {
        if (DEBUG) Log.d(TAG, String.format("vCard export (id: %d) has started.", mJobId));
        ...
        try {
            if (isCancelled()) {
                Log.i(TAG, "Export request is cancelled before handling the request");
                return;
            }
            ...
            final int vcardType;
            Log.d("jypark", TAG + " -> runInternal: exportType=" + exportType);
            ...
            // 2022.12.06 jypark Mantis: 11982
            do {
                if (isCancelled()) {
                ...
                try {
                    String entry = composer.createOneEntry();
                    writer.write(entry);
                } catch (IOException e) {
                    final String errorReason = composer.getErrorReason();
            ...
            Log.i(TAG, "Successfully finished exporting vCard " + request.destUri);

            if (DEBUG) {
                Log.d(TAG, "Ask MediaScanner to scan the file: " + request.destUri.getPath());
            }
    ...
    private void doCancelNotification() {
        if (DEBUG) Log.d(TAG, "send cancel notification");
    ...
    private void doFinishNotification(final String title, final String description) {
        if (DEBUG) Log.d(TAG, "send finish notification: " + title + ", " + description);
    ...
    private void doFinishNotificationWithShareAction(final String title, final String
        ...
        final Notification notification =
                NotificationImportExportListener.constructFinishNotification(
                        mService, title, description, intent); //CVE-2020-0422
        mNotificationManager.notify(NotificationImportExportListener.DEFAULT_NOTIFICATION_TAG,
                mJobId, notification);
    }
    ...
    public synchronized boolean cancel(boolean mayInterruptIfRunning) {
        if (DEBUG) Log.d(TAG, "received cancel request");
        if (mDone || mCanceled) {
            return false;
        }
        mCanceled = true;
