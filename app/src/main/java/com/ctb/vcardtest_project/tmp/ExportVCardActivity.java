...
public class ExportVCardActivity extends Activity implements ServiceConnection,
    ...
    private static final String TAG = "VCardExport";
    ...
    protected void onCreate(Bundle bundle) {
        ...
        if (!hasExportIntentHandler()) {
            Log.e(TAG, "Couldn't find export intent handler");
            showErrorDialog();
            return;
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CREATE_DOCUMENT) {
            if (resultCode == Activity.RESULT_OK && mService != null &&
                ...
                try{
                    final ContentResolver resolver = ExportVCardActivity.this.getContentResolver();
                    InputStream is = resolver.openInputStream(targetFileName);
    
                    InputStreamReader isReader = new InputStreamReader(is);
                    Stream<String> streamOfString = new BufferedReader(isReader).lines();
                    String streamToString = streamOfString.collect(Collectors.joining());

                }
                catch(IOException e){
                    Log.e("jypark", TAG + " -> onActivityResult: e", e);
                }                
                
                if (DEBUG) Log.d(TAG, "exporting to " + targetFileName);
                ...
                if (mService == null) {
                    Log.d(TAG, "No vCard service.");
                } else {
                    Log.d(TAG, "create document cancelled or no data returned");
                }
            }
            finish();
        }
    ...
    public synchronized void onServiceConnected(ComponentName name, IBinder binder) {
        if (DEBUG) Log.d(TAG, "connected to service, requesting a destination file name");
        mConnected = true;
    ...
    public synchronized void onServiceDisconnected(ComponentName name) {
        ...
        if (mProcessOngoing) {
            ...
            Log.w(TAG, "Disconnected from service during the process ongoing.");
            showErrorDialog();
        }
    }
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (DEBUG) Log.d(TAG, "ExportVCardActivity#onClick() is called");
        finish();
    }

    @Override
    public void onCancel(DialogInterface dialog) {
        if (DEBUG) Log.d(TAG, "ExportVCardActivity#onCancel() is called");
        mProcessOngoing = false;
        finish();
    }
