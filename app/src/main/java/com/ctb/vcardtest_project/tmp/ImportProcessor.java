...
public class ImportProcessor extends ProcessorBase implements VCardEntryHandler {
    ...
    public void run() {
        ...
        try {
            ...
            FeedbackHelper.sendFeedback(mService, TAG, "Vcard import failed", e);
        } finally {
            synchronized (this) {
                mDone = true;
            }
    ...
    private void runInternal() {
        Log.i(TAG, String.format("vCard import (id: %d) has started.", mJobId));
        final ImportRequest request = mImportRequest;
        if (isCancelled()) {
            Log.i(TAG, "Canceled before actually handling parameter (" + request.uri + ")");
            return;
        }
        ...
        Log.d("jypark", TAG + " -> runInternal: uri=" + uri + ", account=" + account + ", estimatedVCardType=" + estimatedVCardType + 
                ", estimatedCharset=" + estimatedCharset + ", entryCount=" + entryCount);
        ...
        if (successful) {
            ...
            // value
            if (isCancelled()) {
                Log.i(TAG, "vCard import has been canceled (uri: " + uri + ")");
                // Cancel notification will be done outside this method.
            } else {
                Log.i(TAG, "Successfully finished importing one vCard file: " + uri);
                ...
                if (mListener != null) {
                    if (uris != null && uris.size() == 1) {
                        ...
                        if (uris == null || uris.size() == 0) {
                            ...
                            Log.w(TAG,  "Created Uris is null or 0 length " +
    ...
    private boolean readOneVCard(InputStream is, int vcardType, String charset,
        ...
        for (int i = 0; i < length; i++) {
            ...
            try {
                ...
                FeedbackHelper.sendFeedback(mService, TAG, "Failed to read vcard", e);
    ...
    public synchronized boolean cancel(boolean mayInterruptIfRunning) {
        if (DEBUG) Log.d(TAG, "ImportProcessor received cancel request");
        if (mDone || mCanceled) {
            return false;
        }
        mCanceled = true;
