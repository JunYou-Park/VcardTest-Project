...
import com.android.contacts.vcard.ReadVCardProcessor;
import com.android.contacts.view.LetterImageView;
import com.android.contacts.view.ContactAvatarDrawable;
import com.android.contacts.dialog.MultiContactListDialog;


import static com.android.contacts.util.GraphicUtils.getCircularBitmap;
...
public class ImportVCardActivity extends AppCompatActivity implements ImportVCardDialogFragment.Listener {
    ...
    private final int EMAIL_TYPE = 2;
    private final int MULTI_TYPE = 3;

    private final Handler mCallbackHandler = new Handler(Looper.getMainLooper()){
        @Override
        public void handleMessage(@NonNull Message msg) {
            super.handleMessage(msg);
            ArrayList<VCardEntry> entries = (ArrayList<VCardEntry>) msg.obj;
            if(entries == null || entries.size() == 0) {
                Toast.makeText(getApplicationContext(), R.string.message_invalid_format, Toast.LENGTH_SHORT).show();
                finish();
            }
            if(mAdapter != null) mAdapter.update(entries);
            if(mBtnSave!=null) mBtnSave.setEnabled(true);
            if(mLineLoading!=null) mLineLoading.setVisibility(View.GONE);
        }
    };
    ...
    private MultiContactListDialog mMultiContactDialog;
    private Uri mSourceUri = null;
    private String sourceDisplayName = null;
    private final VCardContactListAdapter mAdapter = new VCardContactListAdapter();
    @Override
...
/*
            ...
            if(mSourceUri==null){
                startImport(mSourceUri, sourceDisplayName);
...
*/
    ...
    private void initView(){
        ...
        mRvList = findViewById(R.id.rv_import_vcard_list);
        mTb = findViewById(R.id.tb_import_vcard);
        mBtnSave = findViewById(R.id.btn_import_vcard_save);
        mLineLoading = findViewById(R.id.line_import_vcard_loading);
        mLineLoading.setVisibility(View.GONE);
        mRvList.setAdapter(mAdapter);
        if(mSourceUri == null)
           mSourceUri = getIntent().getData();
        Log.d(TAG, "init: sourceUri=" + mSourceUri);
        ...
        if (isStorageUri(mSourceUri) && RequestImportVCardPermissionsActivity
        ...
        if(mSourceUri==null) {
            ...
            ReadVCardProcessor readVCardProcessor = new ReadVCardProcessor(this, mCallbackHandler);
            readVCardProcessor.initialize(mSourceUri);
            ...
            mBtnSave.setOnClickListener(v->{
                if (mSourceUri != null) {
                    ...
                    if (TextUtils.isEmpty(localTmpFileName)) {
                        localTmpFileName = readUriToLocalFile(mSourceUri);
                        sourceDisplayName = getDisplayName(mSourceUri);
                        if (localTmpFileName == null) {
                            Log.e(TAG, "Cannot load uri to local storage.");
                    ...
                    mSourceUri = Uri.parse(getFileStreamPath(localTmpFileName).toURI().toString());
                }
            ...
            if (isCallerSelf(this)) {
                ...
                ImportVCardDialogFragment.show(this, mSourceUri, sourceDisplayName);
            }
            */
            });
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
        ...
        if (requestCode == PERMISSIONS_REQUEST_CODE) {
            ...
            if(denied){
                Toast.makeText(getApplicationContext(), R.string.message_permission_require, Toast.LENGTH_SHORT).show();
                finish();
            }
            else{
                initView();
    ...
    private class VCardContactListAdapter extends RecyclerView.Adapter<VCardContactListViewHolder>{

        private final ArrayList<VCardEntry> mEntries = new ArrayList<>();

        public void update(ArrayList<VCardEntry> entries){
            mEntries.clear();
        ...
        @NonNull
        @Override
        public VCardContactListViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
            return new VCardContactListViewHolder(LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_vcard_contact, viewGroup, false));
        }

        @Override
        public void onBindViewHolder(@NonNull VCardContactListViewHolder viewHolder, int i) {
            viewHolder.init(mEntries.get(i));
        }
    ...
    private class VCardContactListViewHolder extends RecyclerView.ViewHolder{
        private TextView tvName;
        private TextView tvMore;
        private LetterImageView ivAvatar;
        private LinearLayout lineParent;
        private int type = PHONE_TYPE;
        public VCardContactListViewHolder(@NonNull View itemView) {
            super(itemView);
            tvName = itemView.findViewById(R.id.tv_vcard_item_name);
            ivAvatar = itemView.findViewById(R.id.iv_vcard_contact_avatar);
            tvMore = itemView.findViewById(R.id.tv_vcard_item_more);
            lineParent = itemView.findViewById(R.id.line_vcard_contact_avatar);
        }



        public void init(VCardEntry entry){
            String displayName = entry.getDisplayName();
            Log.d(TAG, "init: displayName=" + displayName);
            List<VCardEntry.PhoneData> phoneList = entry.getPhoneList();
            List<VCardEntry.EmailData> emailList = entry.getEmailList();
            tvName.setText((displayName==null || displayName.isEmpty()) ? itemView.getContext().getString(R.string.unknown_name) : displayName);
            boolean isEmptyData = displayName == null || (phoneList == null && emailList == null);
            boolean isFullData = phoneList!=null && emailList!=null;
            setAvatar(entry, displayName, entry.hashCode());
            if(!isEmptyData){
                String content = "";
                if(isFullData){
                    type = MULTI_TYPE;
                    content += PhoneNumberUtils.formatNumber(phoneList.get(0).getNumber(), Locale.getDefault().getCountry()) + 
                            " " + itemView.getContext().getString(R.string.contact_content_summary, (phoneList.size() + emailList.size() -1));
                }
                else if (phoneList != null) {
                    String phoneNumber = phoneList.get(0).getNumber();
                    if(phoneList.size()>1){
                        content = PhoneNumberUtils.formatNumber(phoneNumber, Locale.getDefault().getCountry()) + " "
                                + itemView.getContext().getString(R.string.contact_content_summary, (phoneList.size()-1));
                    }
                    else{
                        content = PhoneNumberUtils.formatNumber(phoneNumber, Locale.getDefault().getCountry()) + " (" + phoneList.get(0).getPhoneLabel(Locale.getDefault()) + ")";
                    }
                    type = PHONE_TYPE;
                }
                else {
                    tvName.setText(displayName);
                    String email = emailList.get(0).getAddress();
                    if(emailList.size()>1){
                        content = email + itemView.getContext().getString(R.string.contact_content_summary, (emailList.size()-1));
                    }
                    else{
                        content = email + " (" + emailList.get(0).getEmailLabel(Locale.getDefault()) + ")";
                    }
                    type = EMAIL_TYPE;
                }
            ...
            lineParent.setOnClickListener(v->{
                if(isEmptyData) return;
                if(type == PHONE_TYPE){
                    boolean isSingle = phoneList.size() <= 1;
                    if(isSingle){
                        Intent dialIntent = new Intent(Intent.ACTION_DIAL);
                        dialIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        dialIntent.setData(Uri.parse("tel:" + phoneList.get(0).getNumber().replaceAll("[^0-9]", "")));
                        startActivity(dialIntent);
                    }
                    else{
                        if(mMultiContactDialog!=null && mMultiContactDialog.isShowing()){
                            mMultiContactDialog.dismiss();
                        }
                        mMultiContactDialog = new MultiContactListDialog(ImportVCardActivity.this, entry);
                        mMultiContactDialog.show();
                    }
                }
                else if(type == EMAIL_TYPE){
                    Intent emailIntent = new Intent(Intent.ACTION_SENDTO);
                    emailIntent.setData(Uri.parse("mailto:" + emailList.get(0).getAddress()));
        ...
        private void setAvatar(VCardEntry entry, String displayName, int hashCode){
            if(entry.getPhotoList()!=null) {
                Bitmap bitmap = BitmapFactory.decodeByteArray(entry.getPhotoList().get(0).getBytes(), 0, entry.getPhotoList().get(0).getBytes().length);
                ivAvatar.setImageBitmap(getCircularBitmap(bitmap));
            }
            else{
                ContactAvatarDrawable drawable = new ContactAvatarDrawable(itemView.getContext(), displayName, hashCode);
...
//            finish();
    ...
    private class VCardCacheThread extends Thread
        ...
        @Override
        public void run() {
            Log.i(TAG, "vCard cache thread starts running.");
            if (mConnection == null) {
            ...
            try {
                ...
                if (mSource != null) {
                    try {
                        ...
                        FeedbackHelper.sendFeedback(ImportVCardActivity.this, TAG,
                ...
                Log.i(TAG, "Finished caching vCard.");
                mWakeLock.release();
                try {
        ...
        @Override
        public void onCancel(DialogInterface dialog) {
            Log.i(TAG, "Cancel request has come. Abort caching vCard.");
            cancel();
        }
    }
    ...
    private Uri copyTo(final Uri sourceUri, String filename) throws IOException {
        Log.i(TAG, String.format("Copy a Uri to app local storage (%s -> %s)",
                sourceUri, filename));
        final Context context = ImportVCardActivity.this;
    ...
    private String readUriToLocalFile(Uri sourceUri) {
        ...
        try {
            ...
            FeedbackHelper.sendFeedback(this, TAG, "Failed to copy vcard to local file", e);
            showFailureNotification(R.string.fail_reason_io_error);
            return null;
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        if (requestCode == SELECT_ACCOUNT) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                mSourceUri = getIntent().getData();
                if (mSourceUri == null) {
                    startImport(mSourceUri, /* sourceDisplayName =*/ null);
                } else {
                    sourceDisplayName = getIntent().getStringExtra(
                            LOCAL_TMP_FILE_NAME_EXTRA);
                    mSourceUri = Uri.parse(getFileStreamPath(sourceDisplayName).toURI().toString());
//                    startImport(localUri, sourceDisplayName);
                    Log.d(TAG, "onActivityResult: SELECT_ACCOUNT, mSourceUri" + mSourceUri +
                            ", sourceDisplayName=" + sourceDisplayName);
                    initView();
                }
            } else {
                if (resultCode != Activity.RESULT_CANCELED) {
                    Log.w(TAG, "Result code was not OK nor CANCELED: " + resultCode);
                }
                finish();
            }
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (clipData != null) {
                    ...
                    if (uris.isEmpty()) {
                        Log.w(TAG, "No vCard was selected for import");
                        finish();
                    } else {
                        Log.i(TAG, "Multiple vCards selected for import: " + uris);
                    ...
                    if (mSourceUri != null) {
                        Log.i(TAG, "vCard selected for import: " + mSourceUri);
                        sourceDisplayName = getDisplayName(mSourceUri);
                        Log.i(TAG, "vCard selected for sourceDisplayName: " + sourceDisplayName);
                        initView();
                    }
                    else {
                        Log.w(TAG, "No local URI for vCard import");
                        finish();
                    }
                }
            } 
    ...
    private void startImport(Uri uri, String sourceDisplayName) {
        Log.d(TAG, "startImport: uri=" + uri + ", sourceDisplayName=" + sourceDisplayName);
        // Handle inbound files
        if (uri != null) {
            Log.i(TAG, "Starting vCard import using Uri " + uri);
            importVCard(uri, sourceDisplayName);
        } else {
            Log.i(TAG, "Start vCard without Uri. The user will select vCard manually.");
            actionOpenDocument();
        }
    }
    ...
    protected Dialog onCreateDialog(int resId, Bundle bundle) {
        if (resId == R.id.dialog_cache_vcard) {
            ...
            if (TextUtils.isEmpty(message)) {
                Log.e(TAG, "Error message is null while it must not.");
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        if (mProgressDialogForCachingVCard != null) {
            Log.i(TAG, "Cache thread is still running. Show progress dialog again.");
            showDialog(R.id.dialog_cache_vcard);
        }
    }
