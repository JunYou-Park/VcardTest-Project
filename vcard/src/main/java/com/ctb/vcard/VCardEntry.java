...
public class VCardEntry {
    ...
    public static class PhoneData implements EntryElement {
        ...
        public static final String [] mKorLabels = new String[]{
                "기타", "집", "휴대전화", "직장", "직장 팩스", "집 팩스", "호출기", "TLX", "콜백", "카폰", "MSG", "ISDN", "대표전화"
        };

        public static final String [] mEngLabels = new String[]{
                "Other", "Home", "Mobile", "Work", "Work fax", "Home fax", "Paper", "TLX", "CALLBACK", "Car", "MSG", "ISDN", "Main"
        };

        public String getPhoneLabel(Locale locale){
            boolean isKorean = locale.getCountry().toUpperCase(Locale.ROOT).equals("KR");
            int type = 0;
            if(mType == 0 && !mLabel.contentEquals("VOICE"))
                return mLabel;
            if(mType < mKorLabels.length){
                type = mType;
            }
            if(isKorean)return mKorLabels[type];
            else return mEngLabels[type];
        }
    ...
    public static class EmailData implements EntryElement {
        ...
        public static final String [] mEngLabels = new String[]{
                "Internet", "Home", "Work", "Other", "Mobile"
        };

        public String getEmailLabel(Locale locale){
            boolean isKorean = locale.getCountry().toUpperCase(Locale.ROOT).equals("KR");
            int type = OTHER;
            if(mType<mKorLabels.length) type = mType;
            if(isKorean) return mKorLabels[type];
            else return mEngLabels[type];
        }
